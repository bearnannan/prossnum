...
- **Token Efficiency**: Use concise communication and strategic file reading. Refer to `caveman` principles for high-frequency interactions.
...
Before editing code, perform a symbolic search or read the `workspace_audit.md`. Confirm the backend state (Supabase vs. Google Sheets).
...
1. **`ui-ux-pro-max-skill`**: Primary for UI quality and design-system adherence.
2. **`Memento-Skills`**: Primary for stability and "Exit 0" build enforcement.
3. **`oh-my-codex`**: For orchestrating complex, multi-repo tasks.
4. **`frontend-design`**: Standard implementation tool for web components.
