...
## 6. Intelligence Layer (GitNexus)
Prossnum is not just a UI; it is an intelligent codebase mapped by [GitNexus](file:///d:/APP/prossnum/GitNexus/gitnexus).
- **Contextual Design**: UI components are mapped to execution flows. Changing a "Stat Card" in the UI can be analyzed for its "blast radius" across the `webapp`.
- **Hybrid Understanding**: The design system is documented in `MASTER.md` but is also understood semantically by the agent through the GitNexus knowledge graph.
- **Workflow**: 
  1. **Query**: Use GitNexus to find relevant components.
  2. **Analyze**: Check impact analysis before refactoring.
  3. **Execute**: Apply design changes using `ui-ux-pro-max` guidelines.
